...
interface RecentlyUpdatedSipsClientProps {
  sips: SIP[];
}
...
export default function RecentlyUpdatedSipsClient({ sips: initialSips }: RecentlyUpdatedSipsClientProps) {
  const router = useRouter();
  const [sips, setSips] = useState(initialSips);
  const [searchTerm, setSearchTerm] = useState('');
  // Default sort for this view is by 'updatedAt' descending
  ...
  const formatDate = useCallback((dateString?: string) => {
    ...
    // Check if the date is the epoch date (Jan 1, 1970)
  ...
  const handleSort = (key: SortKey) => {
    if (sortKey === key) {
      ...
      setSortOrder(key === 'updatedAt' || key === 'createdAt' || key === 'mergedAt' ? 'desc' : 'asc');
    }
  };
  ...
  const filteredAndSortedSips = useMemo(() => {
    ...
    if (sortKey) {
      filtered.sort((a, b) => {
        ...
        // Date sorting logic: handle undefined/null dates by pushing them to the end for 'desc' and start for 'asc'
        ...
        // Fallback for other types or if one value is undefined
                    ...
                    return (
                      <Tooltip key={sip.id}>
                        <TooltipTrigger asChild>
                          <TableRow onClick={() => handleRowClick(sip.id)} className="cursor-pointer hover:bg-muted/50 dark:hover:bg-muted/30 transition-colors duration-150 ease-in-out">
